...
import { fetchConfig, isUsingMockData } from '@/lib/api';
import { Users, FileText, Settings, Shield, Plus, Pencil, Trash2 } from 'lucide-react';
...
export default function ConfiguracionPage() {
    ...
    const [config, setConfig] = useState<ConfigData>({ clientes: [], companias: [], plantillas: {} });
    ...
    useEffect(() => {
        async function loadData() {
            if (isUsingMockData()) {
                console.log('[Configuración] Usando datos mock');
                setConfig({
                    clientes: ['CORPORACION ACEROS AREQUIPA', 'GRUPO GLORIA', 'ALICORP'],
                    companias: ['RIMAC', 'PACIFICO', 'MAPFRE', 'LA_POSITIVA', 'SANITAS'],
                    plantillas: {
                        RIMAC: { tiposSeguro: ['SCTR', 'VIDA_LEY'], activo: true },
                        PACIFICO: { tiposSeguro: ['SCTR', 'VIDA_LEY'], activo: true },
                        MAPFRE: { tiposSeguro: ['SCTR'], activo: true }
                    }
                });
                setLoading(false);
                setUsingMock(true);
            ...
            try {
                console.log('[Configuración] Cargando desde API...');
                ...
                setUsingMock(false);
                console.log('[Configuración] Cargados');
            } catch (error) {
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                ...
                                <div className="p-4 bg-gray-50 rounded-lg">
                                    ...
                                    <p className="text-lg font-semibold text-green-600">
                                        {usingMock ? '⚠️ Sin conexión' : '✓ Conectado'}
                                    </p>
                                </div>
                                <div className="p-4 bg-gray-50 rounded-lg">
